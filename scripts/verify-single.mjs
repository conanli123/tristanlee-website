...
export function verifySingle(root) {
  ...
  assert(!html.includes('src="/src/main.tsx"'), 'Development entry in production')
  const match = html.match(/<script id="site-assets">window\.__SITE_ASSETS__=(.*?)<\/script>/s)
  assert(match, 'Embedded media manifest missing')
  const assets = JSON.parse(match[1])
  assert(assets['media/character.mp4'], 'Hero video missing')
  for (const [rel, uri] of Object.entries(assets)) {
    assert(Buffer.from(uri.split(';base64,')[1], 'base64').equals(readFileSync(join(root, 'public', rel))), `Embedded bytes differ: ${rel}`)
  }
  ...
  assert(Buffer.byteLength(html) < 25 * 1024 * 1024, 'HTML exceeds Cloudflare Pages 25 MiB asset limit; use external video hosting for larger films')
  ...
  console.log(`Verified inline application and ${Object.keys(assets).length} byte-identical media assets`)
}
