...
function collectMedia(dir, prefix = "") {
  return readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (entry.isDirectory())
      return collectMedia(join(dir, entry.name), rel + "/");
...
// The former portfolio media stays on disk for reference; ship only this design's assets.
...
html = html.replace(
  /<link\b(?=[^>]*rel="icon")(?=[^>]*href="\.\/([^"]+)")[^>]*>/g,
  (_, rel) => `<link rel="icon" href="${embedded[rel]}" />`,
);
html = html.replace(
  /<link\b(?=[^>]*rel="stylesheet")(?=[^>]*href="\.\/([^"]+)")[^>]*>/g,
  (_, rel) =>
    `<style>${readFileSync(join(dist, rel), "utf8").replace(/<\/style/gi, "<\\/style")}</style>`,
);
const scripts = [
  ...html.matchAll(
...
const script = readFileSync(join(dist, scripts[0][1]), "utf8").replace(
  /<\/script/gi,
  "<\\/script",
);
...
html = html.replace(
  scripts[0][0],
  () =>
    `<script id="site-assets">window.__SITE_ASSETS__=${JSON.stringify(embedded).replace(/</g, "\\u003c")}</script><script type="module">${script}</script>`,
);
...
console.log(
  `Built single-file/index.html: ${(Buffer.byteLength(html) / 1024 / 1024).toFixed(2)} MB`,
);
console.log("Synced the verified build to the root index.html");
